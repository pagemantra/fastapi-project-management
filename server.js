...
function formatSessionWithCalculatedHours(session) {
  ...
  let totalBreakMinutes = breaks.reduce((sum, b) => {
    // Use duration_minutes if set and break is completed
    if (b.duration_minutes && b.duration_minutes > 0) {
      return sum + b.duration_minutes;
    } else if (b.start_time && b.end_time) {
      // Calculate duration from timestamps for completed breaks without duration_minutes
      ...
      const duration = moment(now).diff(moment(b.start_time), 'minutes');
      return sum + Math.max(0, duration);
    }
    return sum;
  }, 0);
...
app.get('/api/v1/app-updates', authenticate, requireRoles([UserRole.ADMIN]), async (req, res) => {
  try {
...
app.post('/api/v1/app-updates', authenticate, requireRoles([UserRole.ADMIN]), async (req, res) => {
  try {
...
app.put('/api/v1/app-updates/:id', authenticate, requireRoles([UserRole.ADMIN]), async (req, res) => {
  try {
...
app.delete('/api/v1/app-updates/:id', authenticate, requireRoles([UserRole.ADMIN]), async (req, res) => {
  try {
    const updateId = req.params.id;
